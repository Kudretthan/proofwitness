...
export default function DisputedClaimsPage({
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-black tracking-tight text-slate-50">Yanlışlananlar</h1>
        <p className="mt-2 text-slate-400">Topluluk tarafından yanlış veya şüpheli olarak işaretlenen bildirimler.</p>
      </div>
      ...
      {claims.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-slate-700 bg-slate-950/30 px-6 py-14 text-center">
          ...
          <h3 className="text-base font-semibold text-slate-200">Henüz yanlışlanmış bildirim yok.</h3>
        </div>
      ) : (
        <div className="space-y-6">
          {claims.map((claim) => (
